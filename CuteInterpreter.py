...
class CuteInterpreter(object):
    ...
    def run_func(self, func_node):
        ...
        if func_node.type is TokenType.CAR:
            ...
            if not is_quote_list(expr_rhs1):
                print ("cdr error!")
            result = pop_node_from_quote_list(rhs1)
            return create_quote_node(result, list_flag = True)
            ...
            if is_quote_list(expr_rhs1):
                expr_rhs1 = Node(TokenType.LIST, pop_node_from_quote_list(expr_rhs1))
            expr_rhs1.next = pop_node_from_quote_list(expr_rhs2)
            return create_quote_node(expr_rhs1, list_flag = True)
            ...
            #작성
            if rhs1.type is not TokenType.INT:
                return self.FALSE_NODE
            elif rhs1.value == rhs2.value:
                return self.TRUE_NODE
            return self.FALSE_NODE
            ...
            while rhs1.value is not None:
                expr_rhs1 = self.run_expr(rhs1)

                if expr_rhs1 is self.TRUE_NODE:
    ...
    def run_list(self, l_node):
        ...
        if op_code.type is TokenType.TRUE:
            return self.TRUE_NODE
        if op_code.type is TokenType.FALSE:
            return self.FALSE_NODE
        else:
